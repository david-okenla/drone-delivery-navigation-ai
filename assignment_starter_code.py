...
if __name__ == "__main__":
    ...
    print(f"     steps={len(dfs_path)-1}, cost={path_cost(dfs_path)}, nodes expanded={dfs_exp}")
    print()
    print(f"BFS  path: {' -> '.join(bfs_path)}")
    print(f"     steps={len(bfs_path)-1}, cost={path_cost(bfs_path)}, nodes expanded={bfs_exp}")
    print()
    print(f"UCS  path: {' -> '.join(ucs_path)}")
    print(f"     steps={len(ucs_path)-1}, cost={ucs_cost}, nodes expanded={ucs_exp}")
    print()
    print(f"A*   path: {' -> '.join(astar_path)}")
    print(f"     steps={len(astar_path)-1}, cost={astar_cost}, nodes expanded={astar_exp}")
    print("=" * 55)